...
public class JdbcUserDaoTests extends BaseDaoTests {
    protected static final User USER_1 = new User(1, "user1", "user1", "ROLE_USER", "user1");
    protected static final User USER_2 = new User(2, "user2", "user2", "ROLE_USER", "user2");
    private static final User USER_3 = new User(3, "user3", "user3", "ROLE_DOCTOR","Doctor", "user3");
    ...
    public void create_user_with_existing_username() {
        sut.create(USER_1.getUsername(), USER_3.getPassword(), "ROLE_USER", "test");
    }

    @Test(expected = IllegalArgumentException.class)
    public void create_user_with_null_password() {
        sut.create(USER_3.getUsername(), null, "ROLE_USER", "test");
    }

    @Test(expected = DataIntegrityViolationException.class)
    public void create_user_with_null_full_name(){sut.create(USER_2.getUsername(), USER_1.getPassword(), "ROLE_USER", null);}

    @Test
    public void create_user_creates_a_user() {
        User newUser = new User(-1, "new", "user", "ROLE_USER", "test");

        boolean userWasCreated = sut.create(newUser.getUsername(), newUser.getPassword(), "ROLE_USER", "test");
